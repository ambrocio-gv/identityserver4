...
namespace IDS4
{
    ...
    {
        //private readonly TestUserStore _users; // for test users
        ...
        public ExternalController(
            ...
            //TestUserStore users = null //for test users 
            ...
            )
        {
            ...
            //_users = users ?? new TestUserStore(TestUsers.Users); // for test users 
        ...
        public async Task<IActionResult> Callback()
        {
            ...
            var (user, provider, providerUserId, claims) = await FindUserFromExternalProvider(result); //await 
            if (user == null)
            {
                // this might be where you might initiate a custom workflow for user registration
                // in this sample we don't show how that would be done, as our sample implementation
                // simply auto-provisions new external user
                user = await AutoProvisionUser(provider, providerUserId, claims); //await 
            }
        ...
        private async Task<(IdentityUser user, string provider, string providerUserId, IEnumerable<Claim> claims)> FindUserFromExternalProvider(AuthenticateResult result)
        {
            ...
            // find external user
            //var user = _users.FindByExternalProvider(provider, providerUserId); // for test users
